...
import (
	...
	"jiffy_tests/GeneratedPersonApp/controllers"
	"jiffy_tests/GeneratedPersonApp/group/gmcom"
	"jiffy_tests/GeneratedPersonApp/group/gmsrv"
	"jiffy_tests/GeneratedPersonApp/middleware"
...
func (a *AppObj) createControllers() {
	...
	a.personC = controllers.NewPersonController(a.services.Person, *a.services)
}
...
func (a *AppObj) initializeRoutes() {
	...
	// ====================== Person protected routes for standard CRUD access ======================
	pActive, ok = svcActv["Person"]
	if ok && pActive {
		a.router.HandleFunc("/persons", requireUserMw.ApplyFn(a.personC.GetPersons)).Methods("GET").Name("person.GET_SET")
		a.router.HandleFunc("/persons/{cmd:[$]+[a-zA-Z0-9_$=]+}", requireUserMw.ApplyFn(a.personC.GetPersons)).Methods("GET").Name("person.GET_SET_CMD")
		a.router.HandleFunc("/person", requireUserMw.ApplyFn(a.personC.Create)).Methods("POST").Name("person.CREATE")
		a.router.HandleFunc("/person/{id:[0-9]+}", requireUserMw.ApplyFn(a.personC.Get)).Methods("GET").Name("person.GET_ID")
		a.router.HandleFunc("/person/{id:[0-9]+}", requireUserMw.ApplyFn(a.personC.Update)).Methods("PUT").Name("person.UPDATE")
		a.router.HandleFunc("/person/{id:[0-9]+}", requireUserMw.ApplyFn(a.personC.Delete)).Methods("DELETE").Name("person.DELETE")

		//=================================== Person Static Filters ===================================
		// http://127.0.0.1:<port>/persons/name(EQ '<sel_string>')
		a.router.HandleFunc("/persons/name{name:[(]+(?:EQ|eq|LIKE|like)+[ ']+[a-zA-Z0-9_]+[')]+}",
			requireUserMw.ApplyFn(a.personC.GetPersonsByName)).Methods("GET").Name("person.STATICFLTR_ByName")

		// http://127.0.0.1:<port>/persons/name(EQ '<sel_string>')/$count |$limit=n $offset=n ($desc|$asc) $orderby=<col_name>
		a.router.HandleFunc("/persons/name{name:[(]+(?:EQ|eq|LIKE|like)+[ ']+[a-zA-Z0-9_]+[')]+}/{cmd:[$]+[a-zA-Z0-9_$=]+}",
			requireUserMw.ApplyFn(a.personC.GetPersonsByName)).Methods("GET").Name("person.STATICFLTR_CMD_ByName")

		// http://127.0.0.1:<port>/persons/age(EQ 72.43)
		// http://127.0.0.1:<port>/persons/age(LT 110)
		// http://127.0.0.1:<port>/persons/age(GE -43)
		a.router.HandleFunc("/persons/age{age:[(]+(?:EQ|eq|LT|lt|GT|gt)+[ ]+[0-9_]+[)]+}",
			requireUserMw.ApplyFn(a.personC.GetPersonsByAge)).Methods("GET").Name("person.STATICFLTR_ByAge")

		// http://127.0.0.1:<port>/persons/age(EQ 72.43)/$count |$limit=n $offset=n ($desc|$asc) $orderby=<col_name>
		// http://127.0.0.1:<port>/persons/age(LT 110)/$count |$limit=n $offset=n ($desc|$asc) $orderby=<col_name>
		// http://127.0.0.1:<port>/persons/age(GE -43)/$count |$limit=n $offset=n ($desc|$asc) $orderby=<col_name>
		a.router.HandleFunc("/persons/age{age:[(]+(?:EQ|eq|LT|lt|GT|gt)+[ ]+[0-9_]+[)]+}/{cmd:[$]+[a-zA-Z0-9_$=]+}",
			requireUserMw.ApplyFn(a.personC.GetPersonsByAge)).Methods("GET").Name("person.STATICFLTR_CMD_ByAge")

		// http://127.0.0.1:<port>/persons/weight(EQ 72.43)
		// http://127.0.0.1:<port>/persons/weight(LT 110)
		// http://127.0.0.1:<port>/persons/weight(GE -43)
		a.router.HandleFunc("/persons/weight{weight:[(]+(?:EQ|eq|LT|lt|LE|le|GT|gt|GE|ge)+[ ]+[0-9._]+[)]+}",
			requireUserMw.ApplyFn(a.personC.GetPersonsByWeight)).Methods("GET").Name("person.STATICFLTR_ByWeight")

		// http://127.0.0.1:<port>/persons/weight(EQ 72.43)/$count |$limit=n $offset=n ($desc|$asc) $orderby=<col_name>
		// http://127.0.0.1:<port>/persons/weight(LT 110)/$count |$limit=n $offset=n ($desc|$asc) $orderby=<col_name>
		// http://127.0.0.1:<port>/persons/weight(GE -43)/$count |$limit=n $offset=n ($desc|$asc) $orderby=<col_name>
		a.router.HandleFunc("/persons/weight{weight:[(]+(?:EQ|eq|LT|lt|LE|le|GT|gt|GE|ge)+[ ]+[0-9._]+[)]+}/{cmd:[$]+[a-zA-Z0-9_$=]+}",
			requireUserMw.ApplyFn(a.personC.GetPersonsByWeight)).Methods("GET").Name("person.STATICFLTR_CMD_ByWeight")

		// http://127.0.0.1:<port>/persons/valid_license(EQ TRUE)
		a.router.HandleFunc("/persons/valid_license{valid_license:[(]+(?:EQ|eq|NE|ne)+[ ']+(?:true|TRUE|false|FALSE)+[')]+}",
			requireUserMw.ApplyFn(a.personC.GetPersonsByValidLicense)).Methods("GET").Name("person.STATICFLTR_ByValidLicense")

		// http://127.0.0.1:<port>/persons/valid_license(EQ TRUE)/$count |$limit=n $offset=n ($desc|$asc) $orderby=<col_name>
		a.router.HandleFunc("/persons/valid_license{valid_license:[(]+(?:EQ|eq|NE|ne)+[ ']+(?:true|TRUE|false|FALSE)+[')]+}/{cmd:[$]+[a-zA-Z0-9_$=]+}",
			requireUserMw.ApplyFn(a.personC.GetPersonsByValidLicense)).Methods("GET").Name("person.STATICFLTR_CMD_ByValidLicense")
